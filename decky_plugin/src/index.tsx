...
function CollectionTile({ group, onOpen, focusRef }: { group: LibGroup; onOpen: (g: LibGroup) => void; focusRef?: React.Ref<any> }) {
  const [focused, setFocused] = useState(false);
...
function GroupsPanel({ mode, visible, onOpenGroup, svcStatus }:
  ...
  if (mode === 'platform') {
    return (
      ...
      >
        {groups.map((g) => <PlatformTile key={g.key} group={g} onOpen={openGroup} focusRef={tileRef(g.key, focusKey)} />)}
        {scrubOverlay}
      </Focusable>
    );
  }
      ...
      {sections.map((s) => {
        ...
        return (
          <div key={s.title} style={{ marginBottom: '6px' }}>
            ...
            >
              {items.map((g) => <CollectionTile key={g.key} group={g} onOpen={openGroup} focusRef={tileRef(g.key, firstKey)} />)}
            </Focusable>
          </div>
        );
      })}
